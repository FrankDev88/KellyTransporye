...
export class AttendanceController {
  ...
  @ApiTags('Asistencia Estándar (QR)')
  @Post('scan')
  @UsePipes(ZodValidationPipe)
  @ApiOperation({
    summary: 'Check-In vía Código QR',
    description: '🔐 **Roles Permitidos:** `DRIVER`\n\nRegistra el abordaje del niño validando la lectura de su gafete QR. El autobús debe estar dentro de la geocerca permitida (50m).'
  })
  @ApiResponse({ status: 201, description: 'Abordaje confirmado y registrado en auditoría.' })
  @ApiResponse({ status: 400, description: 'Error de negocio: Fuera de geocerca, ruta inactiva o niño no pertenece a la ruta.' })
  ...
  @ApiOperation({
    ...
    description: '🔐 **Roles Permitidos:** `DRIVER`\n\nFinaliza el viaje del niño al llegar a su destino (escuela o casa). Requiere que el estado previo del niño sea ON_BOARD y estar en zona.'
  })
  ...
  @ApiTags('Contingencia (Gafete Perdido)')
  @Post('manual')
  @UsePipes(ZodValidationPipe)
  @ApiOperation({
    summary: 'Check-In Manual',
    description: '🔐 **Roles Permitidos:** `DRIVER`\n\nProtocolo especial para registrar el abordaje de un niño que no cuenta con su gafete QR. Exige registrar un motivo (Extravío, Daño, Olvido).'
  })
  ...
  @ApiOperation({
    ...
    description: '🔐 **Roles Permitidos:** `DRIVER`\n\nFinaliza manualmente el viaje del niño que subió sin QR. Exige registrar el motivo de la operación para control estricto.'
  })
